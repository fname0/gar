...
return new class extends Migration {
    public function up(): void
    {
        ...
        {
        $sql = "
drop view if exists gar.gar_data_by_uuid_".$regionCode.";

create view gar.gar_data_by_uuid_".$regionCode." as
...
FROM gar.houses_".$regionCode." h
         LEFT JOIN gar.adm_hierarchies_".$regionCode." ah ON ah.object_id = h.object_id AND ah.is_active = true
         LEFT JOIN gar.adm_hierarchies_".$regionCode." ah1 ON ah1.object_id = ah.parent_obj_id AND ah1.is_active = true
         LEFT JOIN gar.addr_objs_".$regionCode." ao ON ao.object_id = ah.parent_obj_id AND ao.is_active = true
         LEFT JOIN gar.addr_objs_".$regionCode." ao1 ON ao1.object_id = ah1.parent_obj_id AND ao1.is_active = true
         ...
         LEFT JOIN gar.house_params_".$regionCode." hp ON hp.object_id = h.object_id AND hp.type_id = 5 AND hp.end_date >= now()
WHERE h.is_actual = true";
    ...
    public function down(): void
    {
        foreach (config('gar.region_code') as $regionCode)
        {
        DB::unprepared('drop view if exists gar.gar_data_by_uuid_'.$regionCode.';');
        }
    }
};
